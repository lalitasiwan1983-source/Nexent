...
export function DecisionSummary({ contract }: DecisionSummaryProps) {
  const { decision, allowed, confidence, attempt, maxAttempts = 2 } = contract;
  const confidencePercent = Math.round(confidence * 100);

  return (
    <div className="space-y-3 pb-4 border-b border-white/[0.06]">
      {/* Top Tag & Policy Status */}
      ...
      <div className="p-4 sm:p-5 rounded-xl bg-[#08090a] border border-white/[0.08] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {/* Metrics Pill Grid */}
        <div className="flex items-center gap-3 sm:gap-4 font-mono text-xs border-t sm:border-t-0 pt-2 sm:pt-0 border-white/[0.06]">
